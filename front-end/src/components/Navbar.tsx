...
export default function Navbar(){
    ...
    return <div className="container ">
        <div className=" sm:flex justify-between sm:flex-row flex-col  p-5 "> 
            ...
            {context?.user? <div className="flex justify-center items-center mt-3 sm:mt-0 gap-5">
                ...
                <button className="sm:text-xl  text-base font-semibold px-2 py-2 sm:px-4 sm:py-2 bg-purple-300 flex items-center justify-center gap-2 rounded-xl text-purple-500" ><CiShare2/>Share Brain</button>
                 <div className="flex justify-center items-center gap-2"><span className=" rounded-full bg-purple-400 text-white uppercase text-xl w-8  h-8 font-bold  inline-flex justify-center items-center">{context?.user.name.charAt(0)}</span><span className="text-xl font-bold capitalize tracking-tighter">{context?.user.name}</span></div>
